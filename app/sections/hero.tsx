"use client";
import { Reveal, Magnetic, Marquee, AmbientCanvas } from "@/components/mechanics";

export function SectionHero() {
  const h1Words = ["СВІЖА", "ПАСТА"];
  const marqueeItems = [
    "PASTA FAMIGLIA",
    "КИЇВ",
    "ВУЛ. АНТОНОВИЧА, 44",
    "20 ХВИЛИН ДО ГОТОВНОСТІ",
    "БОРОШНО SEMOLA",
    "HANDMADE DAILY",
    "ІТАЛІЙСЬКА КУХНЯ"
  ];

  return (
    <section id="hero" className="relative min-h-[100svh] overflow-hidden bg-foreground text-background">
      {/* Background Layer: Grain, Canvas, and Graded Photo */}
      <img 
        src="https://images.pexels.com/photos/407073/breaking-egg-flour-spices-407073.jpeg?auto=compress&cs=tinysrgb&fit=crop&h=1200&w=800" 
        alt="Процес приготування свіжої пасти" 
        className="rp-graded absolute inset-0 h-full w-full object-cover opacity-70 mix-blend-multiply" 
      />
      <div className="absolute inset-0 bg-gradient-to-t from-foreground via-foreground/45 to-foreground/70" />
      <div className="rp-grain absolute inset-0 pointer-events-none" />
      <AmbientCanvas />

      {/* Vertical Locality Label */}
      <span className="absolute right-4 top-28 hidden lg:block text-xs tracking-[0.35em] uppercase text-background/60 [writing-mode:vertical-rl]">
        КИЇВ — АНТОНОВИЧА 44
      </span>

      {/* Main Content: Poster Scale Type */}
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[92rem] flex-col justify-end px-4 pb-28 lg:px-10 lg:pb-16">
        <Reveal delay={0.1}>
          <p className="mb-4 text-sm font-medium uppercase tracking-[0.3em] text-accent">
            ІТАЛІЙСЬКИЙ РЕСТОРАН
          </p>
        </Reveal>

        {/* Trust Line: Google Rating and Reviews */}
        <Reveal delay={0.2}>
          <div className="mb-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-background/75">
            <span className="font-display font-bold">★ 4.9</span>
            <span className="opacity-20 text-xs">/</span>
            <span className="text-[11px] uppercase tracking-wider">127 ВІДГУКІВ У GOOGLE</span>
            <span className="opacity-20 text-xs">•</span>
            <span className="text-[11px] uppercase tracking-wider">ВІДКРИТА КУХНЯ</span>
          </div>
        </Reveal>

        <Reveal delay={0.3}>
          <h1 className="font-display font-extrabold text-background [text-wrap:balance] text-[clamp(3.2rem,11vw,10rem)] max-w-[12ch]">
            {h1Words.map((word, i) => (
              <span 
                key={i} 
                className="word-rise inline-block mr-[0.3em]" 
                style={{ animationDelay: `${0.15 + i * 0.09}s` }}
              >
                {word}{" "}
              </span>
            ))}
          </h1>
        </Reveal>

        <div className="mt-8 flex flex-wrap items-end justify-between gap-6">
          <Reveal delay={0.4}>
            <p className="max-w-md text-base lg:text-lg leading-relaxed text-background/80">
              Готуємо пасту з твердих сортів пшениці та свіжих яєць на ваших очах. 
              Стіл на</p>
          </Reveal>
          
          <Reveal delay={0.5}>
            <Magnetic>
              <a 
                href="tel:+380443332211" 
                className="inline-flex items-center gap-3 bg-accent px-8 py-4 text-base font-semibold text-accent-foreground transition-transform duration-300" 
                style={{ borderRadius: "var(--radius-control)" }}
              >
                ЗАБРОНЮВАТИ СТІЛ
                <span aria-hidden className="text-xl">→</span>
              </a>
            </Magnetic>
          </Reveal>
        </div>
      </div>

      {/* Edge Marquee Layer */}
      <div className="absolute bottom-0 left-0 right-0 z-10 border-t border-background/15 bg-foreground/70 py-4 backdrop-blur-sm">
        <Marquee 
          items={marqueeItems} 
          className="font-display text-xs lg:text-sm uppercase tracking-[0.25em] text-background/70" 
        />
      </div>
    </section>
  );
}